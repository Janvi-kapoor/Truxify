...
async function loadActiveDevices(userId) {
  ...
  try {
    ...
    if (error) {
      logger.error(
        `[FCM] Failed to load active devices for user ${userId}: ${error.message}`,
      );
      return [];
    }
    ...
    logger.error(
      `[FCM] Failed to load active devices for user ${userId}: ${err.message}`,
    );
    return [];
  }
}
...
async function deactivateInvalidDevices(deviceIds, userId, invalidatedTokens) {
  ...
  if (deviceIds.length > 0) {
    try {
      ...
      if (error) {
        logger.error(
          `[FCM] Failed to deactivate invalid devices for user ${userId}: ${error.message}`,
        );
      } else {
        deactivated = deviceIds.length;
        logger.info(
          `[FCM] Deactivated ${deactivated} invalid device(s) for user ${userId}.`,
        );
      }
    } catch (dbErr) {
      logger.error(
        `[FCM] Failed to deactivate invalid devices for user ${userId}: ${dbErr.message}`,
      );
    }
  }
  ...
  if (invalidatedTokens.length > 0) {
    try {
      ...
      logger.error(
        `[FCM] Failed to clear invalid profile FCM token for user ${userId}: ${dbErr.message}`,
      );
    }
  }
...
async function sendBatchWithRetry(chunkTokens, message, userId, chunkIndex) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      const batchResponse = await firebaseAdmin
        .messaging()
      ...
      if (category === "transient" && attempt < MAX_RETRIES - 1) {
        ...
        logger.info(
          `[FCM] Retrying batch ${chunkIndex} after ${delay}ms for user ${userId}`,
        );
  ...
  return measureExecution(
    ...
    async () => {
      if (!firebaseAdmin || !firebaseAdmin.messaging) {
        logger.warn(
          "[FCM] Firebase not configured — skipping push notification",
        );
        return {
          success: false,
      ...
      if (tokens.length === 0) {
        logger.warn(
          `[FCM] No active devices or FCM token for user ${userId} — skipping push notification`,
        );
        return {
          success: false,
          error: "No FCM token",
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        if (sent.rejected) {
          ...
          if (category === "transient") summary.transient += chunk.length;
            ...
            summary.invalidPayload += chunk.length;
        ...
        for (let j = 0; j < responses.length; j++) {
          ...
          if (category === "permanent") {
            ...
            summary.transient += 1;
            logger.warn(
              `[FCM] Temporary FCM failure for user ${userId} — device kept active (code: ${code})`,
            );
          } else if (category === "invalid-payload") {
            summary.invalidPayload += 1;
            logger.warn(
              `[FCM] Invalid payload for user ${userId} — device kept active (code: ${code})`,
            );
          } else {
            summary.unknown += 1;
            logger.warn(
              `[FCM] Unknown FCM error for user ${userId} — device kept active (code: ${code})`,
            );
          }
...
export async function storeDeliveryOtp(orderId, otp, ttlMinutes = 15) {
  return measureExecution("NotificationService.storeDeliveryOtp", async () => {
    if (!supabaseAdmin) {
      logger.error(
        {},
        "[NotificationService] Service-role client not configured — cannot store OTP.",
      );
      return null;
    }
    ...
    if (invalidateError) {
      logger.error(
        { err: invalidateError },
        "[NotificationService] Failed to invalidate existing OTPs",
      );
    }
    ...
    logger.info(
      `[NotificationService] OTP stored for order ${orderId}, expires at ${expiresAt}`,
    );
    return data;
  });
}
...
export async function getActiveDeliveryOtp(orderId) {
  return measureExecution(
    "NotificationService.getActiveDeliveryOtp",
    async () => {
      if (!supabaseAdmin) {
        logger.error(
          {},
          "[NotificationService] Service-role client not configured — cannot read OTP.",
        );
        return null;
      }
      const { data, error } = await supabaseAdmin
      ...
      if (error) {
        logger.error(
          { err: error },
          "[NotificationService] Failed to fetch active OTP",
        );
        return null;
      }
...
export async function verifyDeliveryOtp(otpId) {
  return measureExecution("NotificationService.verifyDeliveryOtp", async () => {
    ...
    if (!supabaseAdmin) {
      logger.error(
        {},
        "[NotificationService] Service-role client not configured — cannot verify OTP.",
      );
      return false;
    }
    const { data, error } = await supabaseAdmin
    ...
    if (!data) {
      logger.warn(
        "[NotificationService] OTP not found or already verified:",
        otpId,
      );
      return false;
    }
...
export async function expireDeliveryOtps(orderId) {
  return measureExecution(
    "NotificationService.expireDeliveryOtps",
    async () => {
      if (!supabaseAdmin) {
        logger.error(
          {},
          "[NotificationService] Service-role client not configured — cannot expire OTPs.",
        );
        return;
      }
      const { error } = await supabaseAdmin
      ...
      if (error) {
        logger.error(
          { err: error },
          "[NotificationService] Failed to expire OTPs",
        );
      }
    },
  );
...
export async function insertNotification(notificationData) {
  ...
  if (!supabaseAdmin) {
    logger.error(
      "[NotificationService] Service-role client not configured — cannot persist notification.",
    );
    return null;
  }
  const { data, error } = await supabaseAdmin
  ...
  if (error) {
    logger.error(
      "[NotificationService] Database insert failed:",
      error.message,
    );
    return null;
  }
    ...
    async () => {
      if (notifType && !ALLOWED_NOTIF_TYPES.has(notifType)) {
        throw new DomainError(400, {
          error: `Invalid notif_type: ${notifType}`,
        });
      }
      ...
      if (supabaseAdmin) {
        try {
          ...
          logger.error(
            `[NotificationService] Database/Persistence error: ${dbErr?.message ?? String(dbErr)}`,
          );
        }
      }
      ...
      try {
        ...
        logger.error(
          ...
          "Unexpected sendFcmNotification error",
        );
...
export async function sendDeliveryOtpNotification(
  customerId,
  orderDisplayId,
  otp,
) {
